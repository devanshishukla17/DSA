...
~Specialised tree like data structure
~Space Complexity-O(n)
~Used to store a dynamic set of strings
~Each node in a trie represents a single character of a string(ascii values[0-225]).
~the structure of a trie is organised in a way that common prefixes of strings are stored only once.
~APPLICATION: Prefix based search,autocomplete feature,spell check, dictionary implementations.
~CHARACTERISTICS:
Efficient searching
Prefix sharing
...
TYPES OF TRIES
1. STANDARD TRIE/PREFIX TRIE(implementation)
2. COMPRESSES TRIE
3. SUFFIX TRIE
4. COMPACT PREFIX TREE/RADIX TRIE
5. MULTI-WAY TRIE
6. X-FAST AND Y-FAST TRIE
7. TERNARY SEARCH TRIE

EXAMPLE: S={bear,bell,bid,bull,stop,stock}
(root is null)

COMPRESSED TRIE:
compress nodes that are not common to any word.
put the compressed strings into a single node.
...
-------------------------------SUFFIX TREE----------------------------
~Specialised tree that stores all the suffixes of a given string.
